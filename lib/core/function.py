...
def validate(config, val_loader, val_dataset, model, model_fine, criterion, criterion_fine, output_dir,
             tb_log_dir, writer_dict=None, wo_model_fine=False, data_type='test'):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
...
def train_transpose(config, train_loader, model, criterion, optimizer, epoch,
          output_dir, tb_log_dir, writer_dict):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
...
def validate_transpose(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None, data_type='test'):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    with torch.no_grad():
        ...
        if writer_dict:
            ...
            # 'valid' if data_type = 'test', 'train' if data_type = 'train'
            if data_type == 'test':
                if isinstance(name_values, list):
                    for name_value in name_values:
                        writer.add_scalars(
