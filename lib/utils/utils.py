...
def save_checkpoint(states, is_best, output_dir,
    ...
    if is_best and 'state_dict' in states:
        torch.save({
            'state_dict': states['best_state_dict'],
            'state_dict_fine': states['best_state_dict_fine']
...
def save_checkpoint_transpose(states, is_best, output_dir,
                    filename='checkpoint.pth'):
    # 保存常規的 checkpoint
    torch.save(states, os.path.join(output_dir, filename))
...
def get_model_summary_transpose(model, *input_tensors, item_length=26, verbose=False):
    """
    :param model:
    :param input_tensors:
    :param item_length:
